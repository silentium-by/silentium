...
{
	private static Logger _log = LoggerFactory.getLogger(AdminEditNpc.class.getName());
	...
	private static boolean storeTradeList(int itemID, int price, int tradeListID, int order)
	{
	...
	private static void showNpcProperty(L2PcInstance activeChar, L2NpcTemplate npc)
	{
		NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		String content = HtmCache.getInstance().getHtm(StaticHtmPath.AdminHtmPath + "editnpc.htm");
	...
	private static void saveNpcProperty(L2PcInstance activeChar, String command)
	{
		String[] commandSplit = command.split(" ");
	...
	private static void updateNpcSkillData(L2PcInstance activeChar, int npcId, int skillId, int level)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE `npc_skills` SET `level`=? WHERE `npcid`=? AND `skillid`=?");
	...
	private static void showNpcSkillAdd(L2PcInstance activeChar, int npcId)
	{
		L2NpcTemplate npcData = NpcTable.getInstance().getTemplate(npcId);
	...
	private static void addNpcSkillData(L2PcInstance activeChar, int npcId, int skillId, int level)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			PreparedStatement statement = con.prepareStatement("INSERT INTO `npc_skills`(`npcid`, `skillid`, `level`) VALUES(?,?,?)");
			statement.setInt(1, npcId);
	...
	private static void deleteNpcSkillData(L2PcInstance activeChar, int npcId, int skillId)
	{
		try (Connection con = DatabaseFactory.getConnection())
		{
			if (npcId > 0)
			{
				PreparedStatement statement = con.prepareStatement("DELETE FROM `npc_skills` WHERE `npcid`=? AND `skillid`=?");
				statement.setInt(1, npcId);
				statement.setInt(2, skillId);
				statement.execute();
				statement.close();
	...
	private static void reloadNpcSkillList(int npcId)
	{
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			// without race
			PreparedStatement statement = con.prepareStatement("SELECT `skillid`, `level` FROM `npc_skills` WHERE `npcid`=? AND `skillid` <> 4416");
			statement.setInt(1, npcId);
