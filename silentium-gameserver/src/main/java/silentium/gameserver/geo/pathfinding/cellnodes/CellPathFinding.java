...
public class CellPathFinding extends PathFinding
{
	...
	protected CellPathFinding()
	{
		try
		{
			String[] array = MainConfig.PATHFIND_BUFFERS.split(";");
	...
	public List<AbstractNodeLoc> findPath(int x, int y, int z, int tx, int ty, int tz, boolean playable)
	{
		...
		boolean debug = playable && MainConfig.DEBUG_PATH;
		...
		if (path.size() < 3 || MainConfig.MAX_POSTFILTER_PASSES <= 0)
		{
			_findSuccess++;
			return path;
		}
		...
		while (playable && remove && path.size() > 2 && pass < MainConfig.MAX_POSTFILTER_PASSES);
	...
	private static FastList<AbstractNodeLoc> constructPath(AbstractNode node)
	{
		...
		{
			if (!MainConfig.ADVANCED_DIAGONAL_STRATEGY && node.getParent().getParent() != null)
			{
