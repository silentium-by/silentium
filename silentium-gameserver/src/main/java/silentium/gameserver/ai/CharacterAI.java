...
public class CharacterAI extends AbstractAI
{
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Set the AI Intention to AI_INTENTION_IDLE</li> <li>Init cast and attack target</li> <li>Stop the actor auto-attack
	 * client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Stop the actor movement server side AND
	 * client side by sending Server->Client packet StopMove/StopRotation (broadcast)</li> <li>Stand up the actor server side AND
	 * client side by sending Server->Client packet ChangeWaitType (broadcast)</li><BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <BR>
	 * <li>Set the AI Intention to AI_INTENTION_ACTIVE</li> <li>Init cast and attack target</li> <li>Stop the actor auto-attack
	 * client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Stop the actor movement server side AND
	 * client side by sending Server->Client packet StopMove/StopRotation (broadcast)</li> <li>Launch the Think Event</li><BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <B><U> Actions</U> : </B><BR>
	 * <BR>
	 * <li>Stop the actor auto-attack client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Set the
	 * Intention of this AI to AI_INTENTION_ATTACK</li> <li>Set or change the AI attack target</li> <li>Start the actor Auto
	 * Attack client side by sending Server->Client packet AutoAttackStart (broadcast)</li> <li>Launch the Think Event</li><BR>
	 * <BR>
	 ...
	 * <B><U> Actions</U> : </B><BR>
	 * <BR>
	 * <li>Set the AI cast target</li> <li>Stop the actor auto-attack client side by sending Server->Client packet AutoAttackStop
	 * (broadcast)</li> <li>Cancel action client side by sending Server->Client packet ActionFailed to the L2PcInstance actor</li>
	 * <li>Set the AI skill used by INTENTION_CAST</li> <li>Set the Intention of this AI to AI_INTENTION_CAST</li> <li>Launch the
	 * Think Event</li><BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <BR>
	 * <li>Stop the actor auto-attack server side AND client side by sending Server->Client packet AutoAttackStop (broadcast)</li>
	 * <li>Set the Intention of this AI to AI_INTENTION_FOLLOW</li> <li>Create and Launch an AI Follow Task to execute every 1s</li>
	 * <BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <B><U> Actions</U> : </B><BR>
	 * <BR>
	 * <li>Set the AI pick up target</li> <li>Set the Intention of this AI to AI_INTENTION_PICK_UP</li> <li>Move the actor to Pawn
	 * server side AND client side by sending Server->Client packet MoveToPawn (broadcast)</li><BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <B><U> Actions</U> : </B><BR>
	 * <BR>
	 * <li>Stop the actor auto-attack client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Set the AI
	 * interact target</li> <li>Set the Intention of this AI to AI_INTENTION_INTERACT</li> <li>Move the actor to Pawn server side
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Stop the actor auto-attack client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Stop the
	 * actor movement server side AND client side by sending Server->Client packet StopMove/StopRotation (broadcast)</li> <li>
	 * Break an attack and send Server->Client ActionFailed packet and a System Message to the L2Character</li> <li>Break a cast
	 * and send Server->Client ActionFailed packet and a System Message to the L2Character</li> <li>Launch actions corresponding
	 * to the Event onAttacked (only for AttackableAI after the stunning periode)</li><BR>
	 * <BR>
	 */
	@Override
	 ...
	 * actor movement server side AND client side by sending Server->Client packet StopMove/StopRotation (broadcast)</li> <li>
	 * Break an attack and send Server->Client ActionFailed packet and a System Message to the L2Character</li> <li>Break a cast
	 * and send Server->Client ActionFailed packet and a System Message to the L2Character</li><BR>
	 * <BR>
	 */
	@Override
	 ...
	 * AI_INTENTION_ACTIVE</li> <li>If the object was targeted to attack, stop the auto-attack, cancel target and set the
	 * Intention to AI_INTENTION_ACTIVE</li> <li>If the object was targeted to cast, cancel target and set the Intention to
	 * AI_INTENTION_ACTIVE</li> <li>If the object was targeted to follow, stop the movement, cancel AI Follow Task and set the
	 * Intention to AI_INTENTION_ACTIVE</li> <li>If the targeted object was the actor , cancel AI target, stop AI Follow Task,
	 * stop the movement and set the Intention to AI_INTENTION_IDLE</li><BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the distance between the current position of the L2Character and the target (x,y)</li> <li>If the distance >
	 * offset+20, move the actor (by running) to Pawn server side AND client side by sending Server->Client packet MoveToPawn
	 * (broadcast)</li> <li>If the distance <= offset+20, Stop the actor movement server side AND client side by sending
	 * Server->Client packet StopMove/StopRotation (broadcast)</li><BR>
	 * <BR>
	 ...
	 * <BR>
	 * <li>Stop the actor auto-attack client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Stop the
	 * actor movement server side AND client side by sending Server->Client packet StopMove/StopRotation (broadcast)</li> <li>Set
	 * the Intention of this AbstractAI to AI_INTENTION_ACTIVE</li><BR>
	 * <BR>
	 * <B><U> Example of use </U> :</B><BR>
	 * <BR>
	 * <li>PlayerAI, SummonAI</li><BR>
