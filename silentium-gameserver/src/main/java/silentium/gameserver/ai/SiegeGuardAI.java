...
public class SiegeGuardAI extends CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target instanceof L2Playable)
		{
			...
			// Check if the target isn't invulnerable and GM.
			if (player.isInvul() && player.isGM())
				return false;
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		...
		if (intention == AI_INTENTION_IDLE) // active becomes idle if only a summon is present
		{
			...
			if (!_actor.isAlikeDead())
			{
				...
				if (!((L2Attackable) _actor).getKnownList().getKnownPlayers().isEmpty())
				{
	...
	private void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			for (L2Character target : npc.getKnownList().getKnownCharactersInRadius(_attackRange))
			{
				...
				{
					// Get the hate level of the L2Attackable against this target, and add the attacker to the L2Attackable
					// _aggroList
	...
	private final void factionNotifyAndSupport()
	{
		...
		// Verify his faction.
		final String factionId = ((L2Npc) _actor).getClan();
		if (factionId == null)
			return;
		...
		for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if (!factionId.equals(npc.getClan()))
				continue;
