...
public class SiegeGuardAI extends CharacterAI implements Runnable
{
	 ...
	 *
	 */
	public SiegeGuardAI(L2Character.AIAccessor accessor)
	{
		super(accessor);
		_selfAnalysis.init();
	...
	{
		_log.debug(getClass().getSimpleName() + ": changeIntention(" + intention + ", " + arg0 + ", " + arg1 + ")");
	...
	{
		_log.debug(getClass().getSimpleName() + ": thinkAttack(); timeout=" + (_attackTimeout - GameTimeController.getGameTicks()));
	...
	private final void factionNotifyAndSupport()
	{
		...
		if (((L2Npc) _actor).getClan() == null || target == null)
			return;
		
		if (target.isInvul())
			return; // speeding it up for siege guards
		
		String faction_id = ((L2Npc) _actor).getClan();
		...
		for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if (!faction_id.equals(npc.getClan()))
				continue;
	...
	private void attackPrepare()
	{
		...
		{
			if(sGuard.getCastle().getSiege().checkIsDefender(((L2PcInstance) attackTarget).getClan()))
			{	
				// Cancel the target
