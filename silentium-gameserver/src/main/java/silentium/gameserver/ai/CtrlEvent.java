...
public enum CtrlEvent
{
	...
	EVT_THINK,
	/**
	 * The actor was attacked. This event comes each time a physical or magical attack was done on the actor. NPC may start attack
	 * in responce, or ignore this event if they already attack someone, or change target and so on.
	 */
	EVT_ATTACKED,
	...
	EVT_EVADED,
	/**
	 * An event that previous action was completed. The action may be an attempt to physically or magically hit an enemy, or an
	 * action that discarded attack attempt has finished.
	 */
	EVT_READY_TO_ACT,
	/**
	...
	EVT_USER_CMD,
	/**
	 * The actor arrived to assigned location, or it's a time to modify movement destination (follow, interact, random move and
	 * others intentions).
	 */
	EVT_ARRIVED,
	/**
	 * The actor arrived to an intermidiate point, and needs revalidate destination. This is sent when follow/move to pawn if
	...
	EVT_FORGET_OBJECT,
	/**
	 * Attempt to cancel current step execution, but not change the intention. For example, the actor was putted into a stun, so
	 * it's current attack or movement has to be canceled. But after the stun state expired, the actor may try to attack again.
	 * Another usage for CANCEL is a user's attempt to cancel a cast/bow attack and so on.
	 */
	EVT_CANCEL,
