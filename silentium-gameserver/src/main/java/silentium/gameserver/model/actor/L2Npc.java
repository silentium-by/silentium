...
import silentium.gameserver.model.actor.instance.L2TeleporterInstance;
import silentium.gameserver.model.actor.instance.L2WarehouseInstance;
...
/**
 * This class represents a Non-Player-Character in the world. It can be a monster or a friendly character. It also uses a template to fetch some
 * static values. The templates are hardcoded in the client, so we can rely on them.<BR>
 * <BR>
 * L2Character :<BR>
 * <BR>
...
public class L2Npc extends L2Character
{
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Call the L2Character constructor to set the _template of the L2Character (copy skills from template to object and link _calculators to
	 * NPC_STD_CALCULATOR)</li> <li>Set the name of the L2Character</li> <li>Create a RandomAnimation Task that will be launched after the
	 ...
	 * L2PcInstance player (display the select window)</li> <li>If L2Npc is autoAttackable, send a Server->Client packet StatusUpdate to the
	 * L2PcInstance in order to update L2Npc HP bar</li> <li>Send a Server->Client packet ValidateLocation to correct the L2Npc position and
	 * heading on the client</li><BR>
	 * <BR>
	 * <B><U> Actions on second click on the L2Npc (Attack it/Intercat with it)</U> :</B><BR>
	 * <BR>
	 * <li>Send a Server->Client packet MyTargetSelected to the L2PcInstance player (display the select window)</li> <li>If L2Npc is
	 * autoAttackable, notify the L2PcInstance AI with AI_INTENTION_ATTACK (after a height verification)</li> <li>If L2Npc is NOT autoAttackable,
	 * notify the L2PcInstance AI with AI_INTENTION_INTERACT (after a distance verification) and show message</li> <BR>
	 * <BR>
	 ...
	 * <li>Set the L2Npc as target of the L2PcInstance player (if necessary)</li> <li>Send a Server->Client packet MyTargetSelected to the
	 * L2PcInstance player (display the select window)</li> <li>If L2Npc is autoAttackable, send a Server->Client packet StatusUpdate to the
	 * L2PcInstance in order to update L2Npc HP bar</li> <li>Send a Server->Client NpcHtmlMessage() containing the GM console about this L2Npc</li>
	 * <BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Each group of Server->Client packet must be terminated by a ActionFailed packet in order to avoid
	 * that client wait an other packet</B></FONT><BR>
	 ...
	 * <BR>
	 * <li>if the file exists on the server (page number = 0) : <B>../npc/default/12006.htm</B> (npcId-page number)</li> <li>if the file exists
	 * on the server (page number > 0) : <B>../npc/default/12006-1.htm</B> (npcId-page number)</li> <li>if the file doesn't exist on the server :
	 * <B>../npc/npcdefault.htm</B> (message : "I have nothing to say to you")</li><BR>
	 * <BR>
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	...
	public String getHtmlPath(int npcId, int val)
	{
		...
		final String temp = StaticHtmPath.DefaultHtmPath + pom + ".htm";
	...
	public static void showQuestChooseWindow(L2PcInstance player, L2Npc npc, Quest[] quests)
	{
		...
		for (Quest q : quests)
		{
			...
			StringUtil.append(sb, "<a action=\"bypass -h npc_", String.valueOf(npc.getObjectId()), "_Quest ", q.getName(), "\">[", q.getDisplayName());
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the text of the quest state in the folder ../quests/questId/stateId.htm</li> <li>Send a Server->Client NpcHtmlMessage containing
	 * the text of the L2NpcInstance to the L2PcInstance</li> <li>Send a Server->Client ActionFailed to the L2PcInstance in order to avoid that
	 * the client wait another packet</li><BR>
	 * <BR>
	 * 
	 * @param player
	 *            The L2PcInstance that talk with the L2NpcInstance
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the text of the selected HTML file in function of the npcId and of the page number</li> <li>Send a Server->Client NpcHtmlMessage
	 * containing the text of the L2Npc to the L2PcInstance</li> <li>Send a Server->Client ActionFailed to the L2PcInstance in order to avoid
	 * that the client wait another packet</li><BR>
	 * 
	 * @param player
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the range level in wich player must be to obtain buff</li> <li>If player level is out of range, display a message and return</li>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Create a DecayTask to remove the corpse of the L2Npc after 7 seconds</li> <li>Set target to null and cancel Attack or Cast</li> <li>
	 * Stop movement</li> <li>Stop HP/MP/CP Regeneration task</li> <li>Stop all active skills effects in progress on the L2Character</li> <li>
	 * Send the Server->Client packet StatusUpdate with current HP and MP to all other L2PcInstance to inform</li> <li>Notify L2Character AI</li><BR>
	 * <BR>
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove the L2Npc from the world when the decay task is launched</li> <li>Decrease its spawn counter</li> <li>Manage Siege task
	 * (killFlag, killCT)</li><BR>
	 * <BR>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove the L2Npc from the world and update its spawn object</li> <li>Remove all L2Object from _knownObjects and _knownPlayer of the
	 * L2Npc then cancel Attak or Cast and notify AI</li> <li>Remove L2Object object from _allObjects of L2World</li><BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T SEND Server->Client packets to players</B></FONT><BR>
	 * <BR>
	 */
