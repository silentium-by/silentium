		...
		else if (command.startsWith("SevenSigns"))
		{
			...
			switch (val)
			{
				...
				case 33: // "I want to participate" request
					...
					else if (cabal == SevenSigns.CABAL_DUSK && EventsConfig.ALT_GAME_CASTLE_DUSK) // dusk
					{
					...
					else if (cabal == SevenSigns.CABAL_DAWN && EventsConfig.ALT_GAME_CASTLE_DAWN) // dawn
					{
				...
				case 34: // Pay the participation fee request
					...
					L2ItemInstance certif = player.getInventory().getItemByItemId(6388); // Lord of the Manor's Certificate of
																							// Approval
					boolean fee = true;
				...
				case 4: // Join a Cabal - SevenSigns 4 [0]1 x
					...
					{
						if (cabal == SevenSigns.CABAL_DUSK && EventsConfig.ALT_GAME_CASTLE_DUSK)
						{
							if (player.getClan() != null && player.getClan().hasCastle()) // even if in htmls is said that ally
																							// can have castle too, but its not
							{
								showChatWindow(player, SevenSigns.SEVEN_SIGNS_HTML_PATH + "signs_33_dusk_no.htm");
								return;
							}
				...
				case 21:
					...
					switch (contribStoneId)
					{
						...
						case SevenSigns.SEAL_STONE_RED_ID:
							redContrib = (EventsConfig.ALT_MAXIMUM_PLAYER_CONTRIB - score) / SevenSigns.RED_CONTRIB_POINTS;
				...
				case 6: // Contribute Seal Stones - SevenSigns 6 x
					...
					if (contribScore == EventsConfig.ALT_MAXIMUM_PLAYER_CONTRIB)
					{
