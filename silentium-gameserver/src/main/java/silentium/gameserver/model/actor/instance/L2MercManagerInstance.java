...
public final class L2MercManagerInstance extends L2NpcInstance
{
	...
	{
		String filename = StaticHtmPath.MercManagerHtmPath + "mercmanager-no.htm";

		int condition = validateCondition(player);
		if (condition == COND_BUSY_BECAUSE_OF_SIEGE)
			filename = StaticHtmPath.MercManagerHtmPath + "mercmanager-busy.htm"; // Busy because of siege
		else if (condition == COND_OWNER) // Clan owns castle
			filename = StaticHtmPath.MercManagerHtmPath + "mercmanager.htm"; // Owner message window
