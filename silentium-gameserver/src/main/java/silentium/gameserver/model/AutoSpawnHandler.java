...
public class AutoSpawnHandler
{
	 ...
	 *            (If < 0 = default value)
	 * @param despawnDelay
	 *            (If < 0 = default value or if = 0, function disabled)
	 * @return AutoSpawnInstance spawnInst
	 */
	...
	private class AutoSpawner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				// If random spawning is disabled, the spawn at the next set of co-ordinates after the last. If the index is
				// greater than the number of possible spawns, reset the counter to zero.
				...
				else
				{
					for (int i = 0; i < spawnInst._spawnCount; i++)
					{
						...
						// To prevent spawning of more than one NPC in the exact same spot, move it slightly by a small random
						// offset.
