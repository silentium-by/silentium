...
public class TvTManager
{
	...
	private TvTManager()
	{
		if (TvTConfig.TVT_EVENT_ENABLED)
		{
			...
			_log.info("TvT Event: Started.");
		}
		else
		{
			_log.info("TvT Event: Engine is disabled.");
		}
	}
	...
	public void startReg()
	{
		if (!TvTEvent.startParticipation())
		{
			...
			Announcements.announceToAll("TvT Event: Event was cancelled.");
		...
		{
			Announcements.getInstance();
			Announcements.announceToAll("TvT Event: Registration opened for " + TvTConfig.TVT_EVENT_PARTICIPATION_TIME + " minute(s).");
	...
	public void startEvent()
	{
		...
		{
			Announcements.getInstance();
			Announcements.announceToAll("TvT Event: Event cancelled due to lack of Participation.");
		...
		{
			TvTEvent.sysMsgToAllParticipants("TvT Event: Teleporting participants to an arena in " + TvTConfig.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
	...
	{
		Announcements.getInstance();
		Announcements.announceToAll(TvTEvent.calculateRewards());
		TvTEvent.sysMsgToAllParticipants("TvT Event: Teleporting back to the registration npc in " + TvTConfig.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
		TvTEvent.stopFight();
	...
	class TvTStartTask implements Runnable
	{
		...
		private void announce(long time)
		{
			if (time >= 3600 && time % 3600 == 0)
			{
				...
				{
					Announcements.getInstance();
					Announcements.announceToAll("TvT Event: " + (time / 60 / 60) + " hour(s) until registration is closed!");
				}
			...
			else if (time >= 60)
			{
				...
				{
					Announcements.getInstance();
					Announcements.announceToAll("TvT Event: " + (time / 60) + " minute(s) until registration is closed!");
				}
				else if (TvTEvent.isStarted())
				{
			...
			else
			{
				...
				{
					Announcements.getInstance();
					Announcements.announceToAll("TvT Event: " + time + " second(s) until registration is closed!");
				}
				else if (TvTEvent.isStarted())
				{
