...
public class L2BossZone extends L2ZoneType
{
	...
	/**
	 * Some GrandBosses send all players in zone to a specific part of the zone, rather than just removing them all. If this is the case, this
	 * command should be used. If this is no the case, then use oustAllPlayers().
	 * 
	 * @param x
	 * @param y
	 * @param z
	...
	/**
	 * Occasionally, all players need to be sent out of the zone (for example, if the players are just running around without fighting for too
	 * long, or if all players die, etc). This call sends all online players to town and marks offline players to be teleported (by clearing
	 * their relog expiration times) when they log back in (no real need for off-line teleport).
	 */
