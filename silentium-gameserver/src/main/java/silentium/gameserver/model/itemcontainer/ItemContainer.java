...
public abstract class ItemContainer
{
	 ...
	 * @param enchantLevel
	 * @return
	 * @see silentium.gameserver.model.itemcontainer.ItemContainer#getInventoryItemCount(int, int, boolean)
	 */
	...
	public L2ItemInstance addItem(String process, L2ItemInstance item, L2PcInstance actor, L2Object reference)
	{
		...
		if (olditem != null && olditem.isStackable())
		{
			...
			// Updates database
			if (item.getItemId() == 57 && count < 10000 * MainConfig.RATE_DROP_ADENA)
			{
	...
	public L2ItemInstance addItem(String process, int itemId, int count, L2PcInstance actor, L2Object reference)
	{
		...
		if (item != null && item.isStackable())
		{
			...
			// Updates database
			if (itemId == 57 && count < 10000 * MainConfig.RATE_DROP_ADENA)
			{
				// Small adena changes won't be saved to database all the time
				if (GameTimeController.getGameTicks() % 5 == 0)
					item.updateDatabase();
		...
		else
		{
			for (int i = 0; i < count; i++)
			{
				...
				if (template.isStackable() || !MainConfig.MULTIPLE_ITEM_DROP)
					break;
			}
		}
	 ...
	 *            : String Identifier of process triggering this action
	 * @param objectId
	 *            : int objectid of the item to be transfered
	 * @param count
