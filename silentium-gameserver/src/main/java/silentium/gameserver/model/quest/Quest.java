...
import silentium.gameserver.model.quest.jython.QuestJython;
...
public class Quest extends ManagedScript
{
	...
	/**
	 * The function init_LoadGlobalData is, by default, called by the constructor of all quests. Children of this class can
	 * implement this function in order to define what variables to load and what structures to save them in. By default, nothing
	 * is loaded.
	 */
	...
	/**
	 * The function saveGlobalData is, by default, called at shutdown, for all quests, by the QuestManager. Children of this class
	 * can implement this function in order to convert their structures into <var, value> tuples and make calls to save them to
	 * the database, if needed. By default, nothing is saved.
	 */
	...
	/**
	 * Add a timer to the quest, if it doesn't exist already. If the timer is repeatable, it will auto-fire automatically, at a
	 * fixed rate, until explicitly canceled.
	 *
	 * @param name
	 *            name of the timer (also passed back as "event" in onAdvEvent)
	 * @param time
	...
	public final boolean notifyKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		try
		{
			if (altMethodCall && !(this instanceof QuestJython))
			{
	 ...
	 * <LI><U>"res" ends with string ".html" :</U> an HTML is opened in order to be shown in a dialog box</LI> <LI><U>"res" starts
	 * with "<html>" :</U> the message hold in "res" is shown in a dialog box</LI> <LI><U>otherwise :</U> the message held in
	...
	/**
	 * Insert (or Update) in the database variables that need to stay persistant for this quest after a reboot. This function is
	 * for storage of values that do not related to a specific player but are global for all characters. For example, if we need
	 * to disable a quest-gatekeeper until a certain time (as is done with some grand-boss gatekeepers), we can save that time in
	 * the DB.
	 *
	 * @param var
	 ...
	 * <BR>
	 * <U><I>Actions :</I></U><BR>
	 * The selection of the right record is made with : <LI>charId = qs.getPlayer().getObjectID()</LI> <LI>name =
	 ...
	 * </UL>
	 * </LI> <LI>var : string "&lt;state&gt;" as the name of the variable for the quest</LI> <LI>val : string corresponding at the
	 * ID of the state (in fact, initial state)</LI>
	 *
	 * @param qs
	 *            : QuestState
	 */
	 ...
	 * <U><I>Actions :</I></U><BR>
	 * <LI>Get ID state of the quest recorded in object qs</LI> <LI>Test if quest is completed. If true, add a star (*) before the
	 * ID state</LI> <LI>Save in database the ID state (with or without the star) for the variable called "&lt;state&gt;" of the
	 * quest</LI>
	 *
	 * @param qs
	 *            : QuestState
	...
	/**
	 * Auxiliary function for party quests. Checks the player's condition. Player member must be within 1500 distance from the
	 * npc. If npc is null, 1500 distance condition from the player itself is applied.
	 *
	 * @param player
	 ...
	 * @param value
	 *            : a tuple specifying a quest condition that must be satisfied for a party member to be considered.
	 * @return boolean : True if player matches the specified condition. If the var is null, true is returned (i.e. no condition
	 *         is applied).
	 */
	...
	/**
	 * Auxiliary function for party quests. Note: This function is only here because of how commonly it may be used by quest
	 * developers. For any variations on this function, the quest script can always handle things on its own
	 *
	 * @param player
	 *            : the instance of a player whose party is to be searched
	 * @param npc
	 ...
	 * @param value
	 *            : a tuple specifying a quest condition that must be satisfied for a party member to be considered.
	 * @return List<L2PcInstance> : List of party members that matches the specified condition, empty list if none matches. If the
	 *         var is null, empty list is returned (i.e. no condition is applied). The party member must be within 1500 distance
	 *         from the npc. If npc is null, 1500 distance condition from the player itself is applied.
	 */
	 ...
	 * @param value
	 *            : a tuple specifying a quest condition that must be satisfied for a party member to be considered.
	 * @return L2PcInstance : L2PcInstance for a random party member that matches the specified condition, or null if no match. If
	 *         the var is null, null is returned (i.e. no condition is applied). The party member must be within 1500 distance
	 *         from the npc. If npc is null, 1500 distance condition from the player itself is applied.
	 */
	 ...
	 * @param state
	 *            : the state in which the party member's QuestState must be in order to be considered.
	 * @return List<L2PcInstance> : List of party members that matches the specified condition, or null if no match. If the var is
	 ...
	 *            : the state in which the party member's QuestState must be in order to be considered.
	 * @return L2PcInstance: L2PcInstance for a random party member that matches the specified condition, or null if no match. If
	 *         the var is null, any random party member is returned (i.e. no condition is applied).
	 */
