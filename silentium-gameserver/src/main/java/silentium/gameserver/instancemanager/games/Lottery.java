...
public class Lottery
{
	...
	{
		_number = 1;
		_prize = EventsConfig.ALT_LOTTERY_PRIZE;
		_isSellingTickets = false;
		_isStarted = false;
		_enddate = System.currentTimeMillis();

		if (MainConfig.ALLOW_LOTTERY)
	...
	private class stopSellingTickets implements Runnable
	{
		...
		public void run()
		{
			_log.debug("Lottery: Stopping ticket sell for lottery #" + getId() + ".");
			_isSellingTickets = false;
	...
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			_log.debug("Lottery: Ending lottery #" + getId() + ".");
			...
			int prize4 = count4 * EventsConfig.ALT_LOTTERY_2_AND_1_NUMBER_PRIZE;
			int prize1 = 0;
			int prize2 = 0;
			int prize3 = 0;

			if (count1 > 0)
				prize1 = (int) ((getPrize() - prize4) * EventsConfig.ALT_LOTTERY_5_NUMBER_RATE / count1);

			if (count2 > 0)
				prize2 = (int) ((getPrize() - prize4) * EventsConfig.ALT_LOTTERY_4_NUMBER_RATE / count2);

			if (count3 > 0)
				prize3 = (int) ((getPrize() - prize4) * EventsConfig.ALT_LOTTERY_3_NUMBER_RATE / count3);
			...
			_log.debug("Lottery: Jackpot for next lottery is " + newprize + ".");
	...
	public static int[] checkTicket(int id, int enchant, int type2)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			if (rset.next())
			{
				...
				_log.debug("count: " + count + ", id: " + id + ", enchant: " + enchant + ", type2: " + type2);
			}
