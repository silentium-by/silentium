...
import silentium.gameserver.model.actor.instance.L2SepulcherMonsterInstance;
import silentium.gameserver.model.actor.instance.L2SepulcherNpcInstance;
...
public class FourSepulchersManager
{
	...
	private final int[][] _startHallSpawn = { { 181632, -85587, -7218 }, { 179963, -88978, -7218 }, { 173217, -86132, -7218 }, { 175608, -82296, -7218 } };

	private final int[][][] _shadowSpawnLoc = { { { 25339, 191231, -85574, -7216, 33380 }, { 25349, 189534, -88969, -7216, 32768 }, { 25346, 173195, -76560, -7215, 49277 }, { 25342, 175591, -72744, -7215, 49317 } }, { { 25342, 191231, -85574, -7216, 33380 }, { 25339, 189534, -88969, -7216, 32768 }, { 25349, 173195, -76560, -7215, 49277 }, { 25346, 175591, -72744, -7215, 49317 } }, { { 25346, 191231, -85574, -7216, 33380 }, { 25342, 189534, -88969, -7216, 32768 }, { 25339, 173195, -76560, -7215, 49277 }, { 25349, 175591, -72744, -7215, 49317 } },
			{ { 25349, 191231, -85574, -7216, 33380 }, { 25346, 189534, -88969, -7216, 32768 }, { 25342, 173195, -76560, -7215, 49277 }, { 25339, 175591, -72744, -7215, 49317 } }, };
	...
	protected void timeCalculator()
	{
		...
		_entryTimeEnd = _coolDownTimeEnd + EventsConfig.FS_TIME_ENTRY * 60000l;
		_warmUpTimeEnd = _entryTimeEnd + EventsConfig.FS_TIME_WARMUP * 60000l;
		_attackTimeEnd = _warmUpTimeEnd + EventsConfig.FS_TIME_ATTACK * 60000l;
	}
	...
	public synchronized void tryEntry(L2Npc npc, L2PcInstance player)
	{
		...
		// ID ok
			...
			default:
				if (!player.isGM())
				{
					_log.warn(player.getName() + " tried to cheat in four sepulchers.");
					Util.handleIllegalPlayerAction(player, player.getName() + " tried to enter in four sepulchers with invalid npc id.", MainConfig.DEFAULT_PUNISH);
				}
				return;
		}
		...
		if (EventsConfig.FS_PARTY_MEMBER_COUNT > 1)
		{
			if (!player.isInParty() || player.getParty().getMemberCount() < EventsConfig.FS_PARTY_MEMBER_COUNT)
			{
		...
		else if (EventsConfig.FS_PARTY_MEMBER_COUNT <= 1 && player.isInParty())
		{
			if (!player.getParty().isLeader(player))
			{
				showHtmlFile(player, npcId + "-NL.htm", npc, null);
	...
	private void entry(int npcId, L2PcInstance player)
	{
		...
		if (EventsConfig.FS_PARTY_MEMBER_COUNT <= 1 && player.isInParty())
		{
			List<L2PcInstance> members = new FastList<>();
			for (L2PcInstance mem : player.getParty().getPartyMembers())
			{
	...
	public void deleteAllMobs()
	{
		for (L2Npc mob : _allMobs)
		{
			...
			{
				_log.warn("FourSepulchersManager: Failed deleting mob.", e);
			}
		}
		_allMobs.clear();
	}
	...
	protected void closeAllDoors()
	{
		for (int doorId : _hallGateKeepers.values())
		{
			...
			{
				_log.warn("FourSepulchersManager: Failed closing door", e);
			}
		}
	}
	...
	protected class ManagerSay implements Runnable
	{
		...
		public void run()
		{
			if (_inAttackTime)
			{
				...
				if (tmp.get(Calendar.MINUTE) + 5 < EventsConfig.FS_TIME_ATTACK)
				{
				...
				else if (tmp.get(Calendar.MINUTE) + 5 >= EventsConfig.FS_TIME_ATTACK)
	...
	protected class ChangeEntryTime implements Runnable
	{
		...
		public void run()
		{
			...
			if (_firstTimeRun)
				...
				interval = EventsConfig.FS_TIME_ENTRY * 60000l; // else use stupid method
	...
	protected class ChangeWarmUpTime implements Runnable
	{
		...
		public void run()
		{
			...
			if (_firstTimeRun)
				...
				interval = EventsConfig.FS_TIME_WARMUP * 60000l;
	...
	protected class ChangeAttackTime implements Runnable
	{
		...
		public void run()
		{
			...
			if (_firstTimeRun)
				...
				interval = EventsConfig.FS_TIME_ATTACK * 60000l;
	...
	public void showHtmlFile(L2PcInstance player, String file, L2Npc npc, L2PcInstance member)
	{
		...
		html.setFile(StaticHtmPath.SepulchersHtmPath + file, player);
		if (member != null)
