...
public class DimensionalRiftManager
{
	...
	public void loadSpawns()
	{
		...
		try
		{
			File file = new File(MainConfig.DATAPACK_ROOT + "/data/xml/dimensional_rift.xml");
	...
	public synchronized void start(L2PcInstance player, byte type, L2Npc npc)
	{
		...
		if (party == null)
		{
			showHtmlFile(player, StaticHtmPath.SevenSignsHtmPath + "rift/NoParty.htm", npc);
			return;
		}
		...
		{
			showHtmlFile(player, StaticHtmPath.SevenSignsHtmPath + "rift/NotPartyLeader.htm", npc);
			return;
		}
		...
		if (party.getMemberCount() < EventsConfig.RIFT_MIN_PARTY_SIZE)
		{
			NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
			html.setFile(StaticHtmPath.SevenSignsHtmPath + "rift/SmallParty.htm", player);
			html.replace("%npc_name%", npc.getName());
			html.replace("%count%", Integer.toString(EventsConfig.RIFT_MIN_PARTY_SIZE));
		...
		for (L2PcInstance p : party.getPartyMembers())
		{
			...
			{
				showHtmlFile(player, StaticHtmPath.SevenSignsHtmPath + "rift/NotInWaitingRoom.htm", npc);
				return;
			}
			...
			{
				NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
				html.setFile(StaticHtmPath.SevenSignsHtmPath + "rift/NoFragments.htm", player);
				html.replace("%npc_name%", npc.getName());
	...
	private static int getNeededItems(byte type)
	{
		...
		{
			case 1:
				return EventsConfig.RIFT_ENTER_COST_RECRUIT;
			case 2:
				return EventsConfig.RIFT_ENTER_COST_SOLDIER;
			case 3:
				return EventsConfig.RIFT_ENTER_COST_OFFICER;
			case 4:
				return EventsConfig.RIFT_ENTER_COST_CAPTAIN;
			case 5:
				return EventsConfig.RIFT_ENTER_COST_COMMANDER;
			case 6:
				return EventsConfig.RIFT_ENTER_COST_HERO;
			default:
	...
	{
		showHtmlFile(player, StaticHtmPath.SevenSignsHtmPath + "rift/Cheater.htm", npc);
		if (!player.isGM())
		{
			_log.warn("Player " + player.getName() + "(" + player.getObjectId() + ") was cheating in dimension rift area!");
			Util.handleIllegalPlayerAction(player, "Warning!! Character " + player.getName() + " tried to cheat in dimensional rift.", MainConfig.DEFAULT_PUNISH);
		}
	}
