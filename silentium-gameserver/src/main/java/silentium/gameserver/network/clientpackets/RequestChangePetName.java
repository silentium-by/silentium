...
public final class RequestChangePetName extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		else if (PetDataTable.getInstance().doesPetNameExist(_name, pet.getTemplate().getNpcId()))
		{
			activeChar.sendPacket(SystemMessageId.NAMING_ALREADY_IN_USE_BY_ANOTHER_PET);
			return;
		}
		else if (_name.length() < 2 || _name.length() > 8)
		{
			activeChar.sendPacket(SystemMessageId.NAMING_PETNAME_UP_TO_8CHARS);
			return;
		}
		else if (!Util.isValidName(_name, "^[A-Za-z]{2,8}$"))
		{
