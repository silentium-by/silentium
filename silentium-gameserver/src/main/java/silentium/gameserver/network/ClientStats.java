...
public class ClientStats
{
	...
	public ClientStats()
	{
		BUFFER_SIZE = MainConfig.CLIENT_PACKET_QUEUE_MEASURE_INTERVAL;
	...
	protected final boolean countUnderflowException()
	{
		...
		return _underflowReadsInMin > MainConfig.CLIENT_PACKET_QUEUE_MAX_UNDERFLOWS_PER_MIN;
	}
	...
	{
		return _floodsInMin > MainConfig.CLIENT_PACKET_QUEUE_MAX_FLOODS_PER_MIN;
	}

	private final boolean longFloodDetected()
	{
		return (_totalCount / BUFFER_SIZE) > MainConfig.CLIENT_PACKET_QUEUE_MAX_AVERAGE_PACKETS_PER_SECOND;
	}
	...
	private final synchronized boolean countPacket()
	{
		...
		if (tick - _packetCountStartTick > 1000)
		{
			...
			if (_floodDetected && !longFloodDetected() && _packetsInSecond[_head] < MainConfig.CLIENT_PACKET_QUEUE_MAX_PACKETS_PER_SECOND / 2)
		...
		if (!_floodDetected)
		{
			if (count > MainConfig.CLIENT_PACKET_QUEUE_MAX_PACKETS_PER_SECOND)
				shortFloods++;
