...
public class ConsoleLogLayout extends LayoutBase<ILoggingEvent> {
	...
	public String doLayout(final ILoggingEvent event) {
		return joiner.join(new DateTime(event.getTimeStamp()).toString("HH:mm:ss"),
				event.getLevel().toString(), event.getThreadName(), "in", event.getLoggerName(), "-",
				event.getFormattedMessage(), CRLF);
	}
}